...
1. the working batch exists under `graph/working/imports/<import-id>/`
...
4. working-batch assertion provenance still points at the expected import
...
- `2` when the batch exists but canonical or raw verification fails
...
- `--import-id <uuid>`
  Required. Selects the graph working batch to verify.
